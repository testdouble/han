...
- **[docs/skills/guidance.md](./docs/skills/han.plugin-builder/guidance.md).** Serve the authoritative guidance for building skills, agents, and plugins, or vendor it into the current repository as a path-scoped rule index (`init` / `update`) so it surfaces while editing skill and agent files (opt-in `han.plugin-builder` plugin).
