...
Invoke `han-communication:readability-guidance` to surface the shared readability standard into your context before you write, then draft the overview against it. Read [references/overview-template.md](./references/overview-template.md) and render the structure for the resolved mode, drawing on the explorers' findings and the input from Step 3. The skill writes the overview; the explorers' raw findings are not pasted in.
