...
- **`han.coding`** carries the coding skills (`tdd`, `refactor`, `code-review`, `test-planning`, `investigate`, `coding-standard`). A skill goes here when its job is working directly in code: writing it, reviewing it, testing it, investigating it, or standardizing it. It depends on `han.core` and is bundled by the `han` meta-plugin.
