...
Tell the user the exact `/tmp/` path of the generated documentation so they can open and review it before deciding
whether to publish. State plainly that the content has not been published anywhere yet.
...
If the user keeps it local only, **stop**. Report the `/tmp/` doc path and state clearly that nothing was published to
...
5. **Publish delegated and reported:** when the user chose to publish, `han-atlassian:markdown-to-confluence` created or
   updated the page in the chosen mode and its URL was relayed; when the user declined, only the `/tmp/` doc exists.
