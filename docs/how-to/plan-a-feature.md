...
1. **Run [`/plan-a-feature`](../skills/plan-a-feature.md) with the rough idea and an output folder.** A template that works well:
...
1. **Pick the slice spec you are about to plan.** This step has two paths:
    - **If the feature is phased**, run [`/plan-a-phased-build`](../skills/plan-a-phased-build.md) against the Phase 1 spec, then run `/plan-a-feature` again for the specific phase you are working on:
...
1. **Run [`/plan-implementation`](../skills/plan-implementation.md).** A template that works well:
...
1. **Run [`/plan-work-items`](../skills/plan-work-items.md).** A template that works well:
...
- [`/code-review`](../skills/code-review.md) is the right step after `/tdd` finishes a behavior and before you open a PR.
