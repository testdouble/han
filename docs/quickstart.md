...
- *"Spec the new onboarding flow, then write a stakeholder summary I can share with leadership before we build it."* → [`/plan-a-feature`](./skills/plan-a-feature.md) → [`/stakeholder-summary`](./skills/stakeholder-summary.md) → [`/html-summary`](./skills/html-summary.md) *(optional, for a self-contained HTML version to hand off)*.
