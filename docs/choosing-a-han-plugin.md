...
- **`han.core`.** The heart of the suite. It carries the planning, research, analysis, and documentation skills, plus every agent the skills dispatch. If you install only this, you have the full set of specialists, but not the coding skills: `/code-review`, `/investigate`, `/test-planning`, and `/coding-standard` now ship in `han.coding`. See the [Skills Index](./skills/README.md) for the complete list.
- **`han.coding`.** The coding layer: the skills you reach for while working in code. It adds [`tdd`](./skills/han.coding/tdd.md), which drives a feature or behavior through a BDD-framed red-green-refactor loop and writes the tests and production code into your tree; [`refactor`](./skills/han.coding/refactor.md), which restructures existing code without changing its behavior through a test-gated refactoring loop; [`code-review`](./skills/han.coding/code-review.md), which runs a comprehensive review of the current branch or specified files; [`test-planning`](./skills/han.coding/test-planning.md), which produces a prioritized test plan; [`investigate`](./skills/han.coding/investigate.md), which runs an evidence-based root-cause investigation with adversarial validation of the fix; and [`coding-standard`](./skills/han.coding/coding-standard.md), which creates and updates coding standards. This plugin depends on `han.core` and is bundled by the `han` meta-plugin, so the bundled suite includes it; a core-only install does not.
...
- **Core only** (`han.core`): the planning, research, analysis, and documentation skills, plus every agent. No coding, GitHub, or reporting skills, so no `/tdd`, `/code-review`, `/investigate`, `/test-planning`, or `/coding-standard`.
...
| You installed core only and now want the coding skills (`/tdd`, `/code-review`, `/investigate`, `/test-planning`, `/coding-standard`) | `han.coding` (alongside the core you already have) | `/plugin install han.coding@han` |
