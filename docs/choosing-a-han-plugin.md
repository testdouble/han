...
Han ships as a family of plugins in one marketplace. `han.core`, `han.github`, `han.reporting`, `han.feedback`, and `han.plugin-builder` carry components; `han` is a convenience wrapper that bundles `han.core`, `han.github`, and `han.reporting`.
...
- **`han`.** A meta-plugin with no components of its own. It exists to pull in `han.core`, `han.github`, and `han.reporting`. Installing it is how you ask for the bundled suite in one command. It does not bundle `han.feedback` or `han.plugin-builder`.
...
Swap the second command for `han.core@han` if you chose core only, or name a layer plugin directly with `han.github@han`, `han.reporting@han`, `han.feedback@han`, or `han.plugin-builder@han`. They all resolve from the same marketplace.
