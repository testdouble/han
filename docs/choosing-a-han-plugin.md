...
- **`han.coding`.** The code-writing layer. It adds [`tdd`](./skills/tdd.md), which drives a feature or behavior through a BDD-framed red-green-refactor loop and writes the tests and production code into your tree — the suite's only execution skill. This plugin depends on `han.core` and is bundled by the `han` meta-plugin, so the bundled suite includes it; a core-only install does not.
