...
| I2 | How control returns from a backing skill is the load-bearing unspecified mechanic | JD-001 | Unverified, resolved from guidance | D-2 |
...
| I7 | The prose ladder yields three pieces regardless of size | JD-006 | Escalated | D-11 |
...
| I12 | Granting the question tool at the parent would silently break every child's questions | JD-002 (secondary) | Evidenced | D-5 |
| I13 | The manifest format has no optional-dependency mechanism | S2 | Evidenced | Recorded in D-4's reasoning |
...
| I19 | Five near-identical flag paragraphs risk drifting apart | S7 | Evidenced | D-2, D-4 |
...
defines no criterion for telling them apart. The hopeful reading was wrong, so the criterion had to be authored rather
than lifted.

### Open questions and how each resolved

Eight were raised. One reached the operator.
...
- **How control returns.** The composition guidance documents orchestration composition as supported, tells the caller
  ...
  shared rule file is what makes every stop look alike. Settled as D-2.
...
- **Description budgets.** Measured all six. Settled as D-7.
...
- **Landed in:** [D-11](implementation-decision-log.md#d-11-the-prose-ladder-climbs-per-section-once-the-work-is-long-enough),
  and the user stories in the plan.
