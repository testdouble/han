---
name: spike-guidance-forked
description: OI-3 spike harness — the FORKED (context fork) readability-guidance variant. Same payload as the inline variant but declared to run in a forked context. Use only when a spike consumer skill invokes it.
allowed-tools: Read
context: fork
---

# Readability Guidance (forked variant)

You have invoked the readability-guidance skill to source the shared writing standard before you draft prose. Apply it while you draft, then RETURN to the workflow that called you — this guidance is a means to writing your deliverable, not the deliverable itself.

Emit the line `SPIKE_GUIDANCE_MARKER_5R8_FORKED` once, to confirm the guidance rendered, then continue.
