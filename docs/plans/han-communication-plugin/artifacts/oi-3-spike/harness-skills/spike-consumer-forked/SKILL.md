---
name: spike-consumer-forked
description: OI-3 spike harness — heavy consumer skill, FORKED arm. Builds an incident post-mortem and sources the readability standard mid-workflow via a forked (context fork) guidance skill. Use only when explicitly invoked by the spike harness with a TRIAL_ID and data path.
allowed-tools: Read, Write, Skill
---

# Incident Post-Mortem Builder (forked arm)
...
Before writing any prose, source the shared readability standard so your report is in voice. Invoke the Skill tool with skill name `spike-guidance-forked`. Absorb the standard it surfaces.
