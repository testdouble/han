...
how, and why, plus a scent-line list of its skills and agents that links into the full docs. Those long-form docs now
live inside the plugin they describe ([D1](artifacts/decision-log.md#d1-long-form-docs-move-into-each-plugin-directory)),
next to that plugin's README, rather than gathered in the repository-root `docs/` folder.

The reader who wants a cross-plugin catalog still gets one. The skills index and the agents index stay where they are but
shrink to alphabetized lists, each entry a one-sentence scent line linking to the canonical long-form doc
([D5](artifacts/decision-log.md#d5-indexes-shrink-to-alphabetized-link-lists)). A reader choosing what to install reads
one plugin catalog that also walks the install decision
([D4](artifacts/decision-log.md#d4-plugin-index-folds-into-the-choosing-doc)). A reader who wants to see how the skills
...
the plugin README points to it rather than restating it
([D3](artifacts/decision-log.md#d3-per-plugin-readme-is-a-light-front-door)). The reorganization moves and slims files;
it does not change what any skill or agent does.
...
- **Actors**
  - **A reader browsing the repository on GitHub** — someone evaluating Han, onboarding, or looking up how a skill
    behaves.
  - **A contributor** adding or editing a skill, an agent, or a plugin, who must know where the new doc goes and which
    indexes to update.
...
- **Triggers** — a reader opens the repository root, opens a plugin folder, follows a link from an index, or opens the
  workflows page. A contributor adds a new skill or agent and reaches for the coverage rule.
- **Preconditions** — the reorganization has landed: long-form docs relocated into their plugins, per-plugin READMEs
  written, indexes slimmed, the choosing doc turned into the plugin catalog, the workflows page created, and every
  layout description and cross-reference updated to match.

## Primary Flow

1. The reader lands on the repository-root `README.md`. It names what Han is, lists the plugins, and points to the
   plugin catalog, the skills index, the agents index, and the workflows page.
2. The reader opens a plugin folder (for example `han-coding/`) and reads its `README.md`. The README states what that
   plugin does, how, and why, then lists each of the plugin's skills, and its agents when it owns any, as one-sentence
   scent lines ([D3](artifacts/decision-log.md#d3-per-plugin-readme-is-a-light-front-door)).
...
4. The reader follows a scent line and reads the long-form doc. It remains the one canonical source for when to use the
   skill or agent, what it returns, and how to get the most out of it
   ([D3](artifacts/decision-log.md#d3-per-plugin-readme-is-a-light-front-door)).
5. A reader who wants the cross-plugin catalog opens the skills index (`docs/skills/README.md`) or the agents index
   ...
   plugins ([D5](artifacts/decision-log.md#d5-indexes-shrink-to-alphabetized-link-lists)).
6. A reader deciding what to install reads the plugin catalog (`docs/choosing-a-han-plugin.md`). It lists every plugin
   with a link to that plugin's README and walks the install decision in one place
   ([D4](artifacts/decision-log.md#d4-plugin-index-folds-into-the-choosing-doc)).
7. A reader who wants to see how the skills compose opens the workflows page (`docs/workflows.md`). It describes the
   common composition scenarios and shows each as a flow diagram that GitHub renders inline, with no build step
   ([D6](artifacts/decision-log.md#d6-composition-scenarios-and-diagrams-move-to-a-workflows-doc),
   [T1](artifacts/feature-technical-notes.md#t1-github-renders-mermaid-fenced-blocks-natively)).
...
- **Sequence:** the README omits an owned-agents section and instead notes that the plugin's skills dispatch agents that
  live in `han-core` ([D8](artifacts/decision-log.md#d8-plugins-without-agents-omit-the-owned-agents-section)).
...
| ----------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| An internal link still points at an old `docs/skills/{plugin}/` or `docs/agents/{plugin}/` location    | The link is updated to the doc's new in-plugin location so it resolves. No active cross-reference is left pointing at a moved file.                                            |
| A per-plugin README restates content the long-form doc already owns                                    | The README carries only the scent line and the link; the restated detail is removed so the long-form doc stays the single canonical source ([D3](artifacts/decision-log.md#d3-per-plugin-readme-is-a-light-front-door)). |
| A layout description names the old doc locations                                                       | The description is corrected to the new locations wherever it appears in reader-facing guidance ([D9](artifacts/decision-log.md#d9-layout-descriptions-and-tooling-updated-to-new-paths)).                                |
...
| The documentation-maintenance tooling still audits the old paths                                       | The tooling's path references are updated to the new locations so a documentation audit checks the right files ([D9](artifacts/decision-log.md#d9-layout-descriptions-and-tooling-updated-to-new-paths)).                 |
| A plugin has no skills yet                                                                              | This cannot occur under the current suite; every plugin ships at least one skill, so every README has a skills section.                                                       |

## Coordinations

| Coordinating System                                                            | Direction | Interaction                                                    | Ordering / Consistency Requirement                                                                 |
| ------------------------------------------------------------------------------ | --------- | ------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| Repository-root `README.md`, `CLAUDE.md`, and `CONTRIBUTING.md`                | outbound  | Describe the documentation layout and where docs live         | Their layout descriptions must match the on-disk locations after the move                          |
| Skills index, agents index, and plugin catalog                                 | outbound  | Link into the relocated long-form docs and per-plugin READMEs | Every link must resolve to a file that exists at its new location                                  |
| Long-form doc coverage rule and the skill and agent templates                  | outbound  | State where a new long-form doc is created                    | Their stated paths must point to the in-plugin locations                                           |
| The documentation-maintenance tooling                                          | outbound  | Audits that every skill and agent has a current long-form doc | Its path references and scope mapping must target the new locations                                |
| GitHub Markdown rendering                                                       | inbound   | Renders the workflows page's flow diagrams for the reader     | Diagrams must be authored in the fenced form GitHub renders natively ([T1](artifacts/feature-technical-notes.md#t1-github-renders-mermaid-fenced-blocks-natively)) |
...
- **Outcome delivered:** A reader navigates Han documentation plugin-first, with per-plugin READMEs and in-plugin
  long-form docs, slimmed alphabetized indexes, one plugin catalog, and a workflows page whose diagrams render on GitHub.
...
- **Decisions settled by user input:** 8 — see [artifacts/decision-log.md](artifacts/decision-log.md)
