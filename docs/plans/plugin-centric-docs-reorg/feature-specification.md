...
This reorganization makes Han's documentation plugin-first. Every plugin gets its own README front door, plus its own
skill and agent long-form docs. The catalog indexes shrink to alphabetized link lists, and each concept keeps exactly
one canonical source after the move.

## Outcome

A reader who opens the Han repository can start at any plugin folder and understand that plugin on its own. The folder
shows what the plugin does, how, and why. It also states whether the plugin is bundled or opt-in, and lists its skills
and agents as short scent lines that link into the full docs
([D13](artifacts/decision-log.md#d13-plugin-readme-states-bundled-vs-opt-in-and-dependencies)).

Those long-form docs now live inside the plugin they describe
...
The reader who wants a cross-plugin catalog still gets one.

The skills index and the agents index stay where they are but shrink to alphabetized lists. Each entry is a short scent
line linking to the canonical long-form doc
([D5](artifacts/decision-log.md#d5-indexes-shrink-to-alphabetized-link-lists)).

A reader choosing what to install reads one plugin catalog that doubles as the plugin index and walks the install
decision ([D4](artifacts/decision-log.md#d4-the-choosing-doc-becomes-the-plugin-index)).

A reader who wants to see how the skills chain together reads a workflows page. Its flow diagrams render directly on
GitHub ([D6](artifacts/decision-log.md#d6-composition-scenarios-and-diagrams-move-to-a-workflows-doc)).

Every concept keeps exactly one canonical home. The long-form doc stays the single source of truth for a skill or agent.
The plugin README stays the single source of truth for what its plugin does. The other surfaces point to those homes
rather than restating them ([D3](artifacts/decision-log.md#d3-per-plugin-readme-is-the-canonical-plugin-front-door)).

The reorganization moves and slims files. It does not change what any skill or agent does.
...
- **Preconditions**: the reorganization has landed. Long-form docs are relocated into their plugins, per-plugin
  READMEs are written, indexes are slimmed, the choosing doc is turned into the plugin index, and the workflows page is
  created. Every layout description, authoring standard, and cross-reference is updated to match. A repository-wide
  internal-link check passes with zero unresolved links ([D16](artifacts/decision-log.md#d16-link-integrity-is-the-acceptance-gate)).
...
2. The reader opens a plugin folder (for example, `han-coding/`) and reads its `README.md`. The README states what
   that plugin does, how, and why. It also states whether the meta-plugin bundles the plugin or it is opt-in, and
   lists its dependencies ([D13](artifacts/decision-log.md#d13-plugin-readme-states-bundled-vs-opt-in-and-dependencies)).
   It then lists each of the plugin's skills, and its agents when it owns any, as short scent lines
...
5. A reader who wants the cross-plugin catalog opens the skills index (`docs/skills/README.md`) or the agents index
   ...
   plugins. Each also points across to the workflows page for how those skills chain together
   ([D5](artifacts/decision-log.md#d5-indexes-shrink-to-alphabetized-link-lists)).
...
7. A reader who wants to see how the skills compose opens the workflows page (`docs/workflows.md`). It describes the
   common composition scenarios, with a flow diagram wherever a chain is branching enough that a picture beats the
   prose. GitHub renders those diagrams inline with no build step
...
- **Sequence:** they write the skill's `SKILL.md` in the plugin and add its long-form doc at that plugin's
  `docs/skills/{name}.md`. They also add a scent line to the plugin's README and one alphabetized entry to the skills
  index. The README and index scent lines reuse the long-form doc's own summary line so the three do not drift
...
- **Sequence:** they write the agent definition and add its long-form doc at that plugin's `docs/agents/{name}.md`.
  They also add a scent line to that plugin's README and one alphabetized entry to the agents index.
...
| A reader opens a historical plan or research archive that references an old doc path                 | The archive is left unchanged as a point-in-time record; its stale path is not rewritten ([D10](artifacts/decision-log.md#trivial-decisions)).                                                          |
...
- **Rewriting historical archives.** The plan and research archives under `docs/plans/` and `docs/research/` are
  ...
  ([D10](artifacts/decision-log.md#trivial-decisions)).
- **Changing skill or agent behavior.** This is a documentation move and slim-down. No `SKILL.md` body, agent
  definition, or skill behavior changes ([D11](artifacts/decision-log.md#trivial-decisions)).
...
- **Why deferred:** simpler-version test. GitHub already renders the reorganized Markdown and the workflows page's
  flow diagrams with no build step ([T1](artifacts/feature-technical-notes.md#t1-github-renders-mermaid-fenced-blocks-natively)).
  That satisfies the findability and orientation goal at far lower cost than a published site. The plugin-centric
...
- **OI-1:** Whether `han-core`'s README keeps grouping its skills by purpose (as the current skills index does) or lists
  ...
  - **Resolves when:** the `han-core` README is drafted and the by-purpose grouping is judged against a flat list for
    ...
    [D12](artifacts/decision-log.md#trivial-decisions).
...
- **Outcome delivered:** A reader navigates Han documentation plugin-first, with per-plugin READMEs and in-plugin
  long-form docs. Indexes are slimmed to alphabetized lists, one plugin index doubles as the install guide, and a
  workflows page renders its diagrams on GitHub.
...
- **Key adjustments from review:** the review widened the change scope to the full active blast radius, plus
  relative-link recomputation and a link-integrity gate. It declared the plugin README canonical for plugin purpose
  and slimmed the catalog to a plugin index. It brought the plugin-README standard and the meta-plugin README into
  scope, and it corrected the shared-agent dispatch note. See [artifacts/team-findings.md](artifacts/team-findings.md)
