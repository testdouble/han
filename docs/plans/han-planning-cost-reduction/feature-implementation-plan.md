...
review teams, four of them gain an executed check that design images listed in the boundary record are on disk, one gains
an executed check that a reviewed plan's cross-references resolve, and three stop proofreading text the readability
editor already rewrote. Most of the work is prose editing inside skill instruction files. The part that is not is two new
Bash scripts, and the one thing to know first is that both scripts read values out of documents an earlier run wrote, so
how they read those values is where nearly every decision in this plan went.
...
- **US-2:** As an operator running any of the four skills that gate on design material, I want the design-image check to
  actually run, so that I find out which images are missing instead of reading a claim that they were checked.
- **US-3:** As an operator running `iterative-plan-review`, I want the cross-reference check to actually run, so that a
...
- **Recorded boundary:** [artifacts/scope-boundary.md](artifacts/scope-boundary.md). The boundary is the research report
  ...
  skills for the expert reduction and one skill's step for the cross-reference conversion, and it excludes the question
...
skill's `scripts/` directory and is invoked from the skill body by its expanded skill-directory path, opens with the same
strict-mode and argument-guard preamble the existing screenshot uploader uses, and takes its tests in a `.bats` file of
the same basename sitting beside it
...
comment naming the other three, which is what the repository's authoring guidance requires and what its one existing
...
correct on a run where two boundary records exist, which the specification already settled in favor of the record beside
...
looking for an allow-list while making every refusable row invisible, which is the vacuous pass this feature exists to
remove.

An accepted location has to match an anchored allow-list of permitted characters and accepted file types, after at most
one surrounding pair of backticks is stripped, with the design folder's own name allowed as an optional prefix. Anything
containing a parent-directory reference is refused. The check does not normalize the value, does not resolve it against
...
extracts identifiers with its own anchored patterns, searches for any document-supplied value as a fixed string, and
decides whether it is inside an example block as it walks the file rather than filtering example hits out afterward
...
| 2   | Reduce the domain-expert counts in `iterative-plan-review`                | US-1             | A team-mode review convenes two experts at large and one at medium, with its smallest size still convening no team                                                                                            | The boundary's Stated Scope names this skill's team mode; the counts are derived and carry inherited open item OI-1                                                                      | —          | Read-through; no test, by [D-18](artifacts/implementation-decision-log.md#d-18-the-prose-edit-majority-gets-no-test-and-the-plan-says-so)                        |
...
parallel with the design-image chain but should not be squeezed into the same budget
([D-4](artifacts/implementation-decision-log.md#d-4-the-cross-reference-check-is-one-copy-and-materially-larger-work)).
...
behavior and are covered; two describe states no test can reach, and they are not covered. The larger half of this
feature by volume, the prose edits, gets no tests at all, and this plan states that rather than shipping a suite that
implies otherwise ([D-18](artifacts/implementation-decision-log.md#d-18-the-prose-edit-majority-gets-no-test-and-the-plan-says-so)).
...
extensions, with parent-directory references refused outright, and with no normalization, no filesystem resolution before
validation, and no retry of a repaired variant
([D-6](artifacts/implementation-decision-log.md#d-6-anchored-whole-string-allow-list-on-the-recorded-location)). The
...
syntax. Mitigated by extracting identifiers with the script's own anchored patterns, searching for document-supplied
values as fixed strings with an explicit end-of-options marker before the filename, and toggling example-block state
while walking the file rather than filtering example hits afterward
([D-8](artifacts/implementation-decision-log.md#d-8-the-cross-reference-check-never-builds-a-pattern-from-document-text)).
...
echoing the offending value only where the operator needs it, through one bounded transform that strips control
characters, collapses newlines and tabs, truncates, and emits a single labeled line
...
| A3  | How this host actually expands a skill-directory variable inside a permission prefix and inside a Bash argument | Nothing in this plan; the plan uses per-skill paths and declares no permission, so both halves are already moot      | Runtime-only |
...
that determine which work units exist in this plan, and the other four are recorded in
...
- **OI-1:** The expert counts for `iterative-plan-review` were derived from the counts agreed for `plan-implementation`
  rather than agreed directly, and that skill fills a third seat conditionally, so its medium band already carries no
  more than one expert on a plan that makes claims about code.
  - **Resolves when:** the operator confirms the derived counts or names different ones.
  - **Blocks implementation:** No — the counts are stated in
    [D2](artifacts/decision-log.md#d2-scope-the-reduction-to-the-two-skills-the-boundary-names) and can be adjusted while
    work unit 2 is being done. Inherited from the specification.
...
- **OI-3:** How this host expands a skill-directory variable inside a permission prefix, and inside a Bash argument,
  ...
  - **Blocks implementation:** No — and it is now moot on both halves. The permission half is moot because this plan
...
Ship as planned. Every open item is non-blocking, no specialist named a handoff for implementation, and the four
