...
- **[`readability-editor`](./han-communication/readability-editor.md).** Rewrites a finished draft for a non-author reader against the shared readability standard, preserving every fact and leaving code, diagrams, and citation identifiers untouched. Dispatched by the synthesis skills as their readability rewrite pass. See [Readability](../readability.md).
