...
- You want a repository to carry the plugin-building guidance locally (`/guidance init`) so it loads automatically while editing skill and agent files.
...
- `/guidance init`. *Vendor the guidance into this repo as a path-scoped rule index.*
- `/guidance update`. *Refresh the vendored guidance after updating the plugin.*
