...
[`han-coding/skills/investigate/SKILL.md`](../../../han-coding/skills/investigate/SKILL.md).
...
- **Plan review.** Use [`/iterative-plan-review`](../han-planning/iterative-plan-review.md) for multi-pass review of an
  existing plan.
- **Open-ended research.** Use [`/research`](../han-core/research.md) when nothing is broken and you want options, prior
...
- **For the full end-to-end bug-handling workflow**, including when to triage instead of investigating now and how to
  ...
  [How to triage and investigate a bug](../../how-to/triage-and-investigate-a-bug.md).
...
2. **Document root cause.** The skill writes Problem Statement, Evidence Summary, and Root Cause Analysis into the plan
   file using the template at [`references/template.md`](../../../han-coding/skills/investigate/references/template.md).
...
- [`/issue-triage`](../han-core/issue-triage.md). Run before investigation when the incoming report is too vague to
  trace; triage produces the sharp problem statement investigation needs.
- [`/research`](../han-core/research.md). The question-shaped sibling. Use it when nothing is broken and you want
  options, prior art, or how something works before committing.
- [`evidence-based-investigator`](../../agents/han-core/evidence-based-investigator.md). The agent the skill dispatches
  in parallel for multi-angle evidence gathering.
- [`adversarial-validator`](../../agents/han-core/adversarial-validator.md). The agent that challenges evidence and fix
...
- [Evidence](../../evidence.md). The canonical evidence rule the skill applies to every finding. Codebase findings stand
...
- [`concurrency-analyst`](../../agents/han-core/concurrency-analyst.md),
  [`behavioral-analyst`](../../agents/han-core/behavioral-analyst.md),
  [`data-engineer`](../../agents/han-core/data-engineer.md). Specialist analysts dispatched alongside the investigators
  when the symptom classification calls for them.
- [`/iterative-plan-review`](../han-planning/iterative-plan-review.md). Pair when the fix plan needs further
...
- [`/runbook`](../han-core/runbook.md). Pair after the investigation lands a procedure the team will reuse. Investigate
