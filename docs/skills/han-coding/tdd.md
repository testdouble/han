...
[`han-coding/skills/tdd/SKILL.md`](../../../han-coding/skills/tdd/SKILL.md).
...
- **Producing a test plan without writing code.** Use [`/test-planning`](../han-coding/test-planning.md) instead. It
...
- **Deciding what a feature should do.** Use [`/plan-a-feature`](../han-planning/plan-a-feature.md) to specify behavior
  first, then bring the spec here.
- **Finding the root cause of a bug.** Use [`/investigate`](../han-coding/investigate.md). Once you have a fix in mind,
...
- **Bring a specification when you have one.** `/tdd` builds a better test list from a `feature-specification.md` than
  ...
  [`/plan-a-feature`](../han-planning/plan-a-feature.md) first for anything non-trivial.
- **Have your standards and ADRs discoverable.** The green and refactor steps apply your coding standards and
  ...
  [`/coding-standard`](../han-coding/coding-standard.md) and
  [`/architectural-decision-record`](../han-core/architectural-decision-record.md), the skill finds and applies them. If
...
- **Pair with `/code-review` next.** TDD produces self-testing code; it does not replace a second set of eyes. Run
  [`/code-review`](../han-coding/code-review.md) on the branch when the list is empty.
...
summary. See [YAGNI](../../yagni.md) for the two gates, the acceptable-evidence list, the named anti-patterns, and the
deferral format.
...
[`/iterative-plan-review`](../han-planning/iterative-plan-review.md): front-loaded constraints, a deterministic
per-cycle process, a bounded list. The project and command discovery follows
[`/test-planning`](../han-coding/test-planning.md): resolve from CLAUDE.md's `## Project Discovery`, fall back to
...
- [YAGNI](../../yagni.md). The evidence-based "You Aren't Gonna Need It" rule the refactor step and test list apply. The
...
- [`/test-planning`](../han-coding/test-planning.md). Plan what to test without writing code. Use it before `/tdd` to
  enumerate behaviors, or instead of it when you want analysis rather than implementation.
- [`/plan-a-feature`](../han-planning/plan-a-feature.md). Specify behavior first; the spec becomes the test list `/tdd`
  builds from.
- [`/code-review`](../han-coding/code-review.md). Run it on the branch once the list is empty. TDD produces self-testing
  code; it does not replace review.
- [`/coding-standard`](../han-coding/coding-standard.md) and
  [`/architectural-decision-record`](../han-core/architectural-decision-record.md). The standards and ADRs `/tdd`
