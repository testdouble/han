...
- [`readability-editor`](../../agents/han-communication/readability-editor.md). Dispatched on the consolidated reports (medium and large, where `project-manager` ran) to rewrite the report against the shared readability standard, preserving every fact and gap ID. Skipped at small and on the `no swarm` path.
