...
Enumerate the plugins from `plugins` in Project Context (one parent, plus each child). For **every** plugin, determine `baseline`, whether it changed in `{range}`, and its `target`. Classify changes against [`han.plugin-builder/skills/guidance/references/semantic-versioning.md`](../../../han.plugin-builder/skills/guidance/references/semantic-versioning.md). The governing rules:
